...
echo.
echo [1/3] Booting AI Model Backend (Flask)...
start "Weather AI Backend" cmd /k "title Weather Backend & color 0A & cd backend & if exist venv\Scripts\python.exe (venv\Scripts\python.exe -m pip install -q flask flask-cors pandas openpyxl python-dotenv requests & venv\Scripts\python.exe app.py) else (pip install -q flask flask-cors pandas openpyxl python-dotenv requests & python app.py)"
